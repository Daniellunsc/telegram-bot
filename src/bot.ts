...
class bot {
  ...
  private setMsgListener() {
    this.mybot.onText( /\/lunabot (.*)/, ( msg, match ) => {
      console.log((msg.chat.id != this.allowedChat)&&(this.allowedChat.toString() != '*'))
      if((msg.chat.id != this.allowedChat)&&(this.allowedChat.toString() != '*')){
        return this.sendMessage(msg.chat.id, "Chat not allowed");
      } 
      console.log(match[1])
      switch(match[1]){
        case 'test':
